...
def test():
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        # if 0 <= x < depth_map.shape[1] and 0 <= y < depth_map.shape[0]:
        #     depth = depth_map[y, x]
        #     cv2.putText(disparity_map_norm, "{}, ".format(depth_map), (point[0], point[1] - 100),
        #                 cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), 1)
            # cv2.setWindowTitle(window_name, f"Depth: {depth:.2f} (x={x}, y={y})")
        # else:
            # cv2.setWindowTitle(window_name, window_name)  # reset window title if mouse is outside of the image.
